...
def has_hope(mat):
    ...
    for idx in range(3):
        if mat[idx][0] and mat[idx][1] and mat[idx][2]:
            this_sum = sum(mat[idx])
            if cache_sum and cache_sum != this_sum:
                return False
            else:
                cache_sum = this_sum
        if mat[0][idx] and mat[1][idx] and mat[2][idx]:
            this_sum = mat[0][idx] + mat[1][idx] + mat[2][idx]
            if cache_sum and cache_sum != this_sum:
                return False
            else:
                cache_sum = this_sum
    if mat[0][0] and mat[1][1] and mat[2][2]:
        this_sum = mat[0][0] + mat[1][1] + mat[2][2]
        if cache_sum and cache_sum != this_sum:
            return False
        else:
            cache_sum = this_sum
    if mat[0][2] and mat[1][1] and mat[2][0]:
        this_sum = mat[0][2] + mat[1][1] + mat[2][0]
        if cache_sum and cache_sum != this_sum:
            return False
        else:
            cache_sum = this_sum
